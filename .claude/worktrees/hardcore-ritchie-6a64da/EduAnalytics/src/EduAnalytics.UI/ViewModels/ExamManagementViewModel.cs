...
/// <summary>
/// Sınav yönetimi ekranı: tüm sınavları listeler; üst düzey alanlar düzenlenebilir;
/// sınav silinebilir (cevaplar dahil cascade ile).
/// </summary>
public partial class ExamManagementViewModel : ObservableObject
{
    ...
    /// <summary>Edit dialog'un açılış olayı — analiz/cevap girişine yönlendirme için kullanılabilir.</summary>
    ...
    private async Task DeleteAsync(ExamListItemDto? item)
    {
        ...
        var warning = item.HasAnswers
            ? $"\n\n⚠ Bu sınava {item.TotalAnswers} cevap girilmiş — silinince hepsi kaybolur."
            : string.Empty;
